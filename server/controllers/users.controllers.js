...
const {
  ...
  recoverPassword,
  restorePassword,
} = require("../models/users.models"); // Importar la función createUser
...
async function refreshToken(req, res) {
  ...
  try {
    ...
    // Consultar la base de datos para obtener los datos actualizados del usuario
    const client = await pool.connect();
    const result = await client.query(queries.getUserById, [payload.id]);
    const user = result.rows[0];
    ...
    // Enviar el nuevo access token como cookie
    res
...
const handleRestorePassword = async (req, res) => {
  ...
  try {
    ...
    if (payload.email !== email) {
      return res
        .status(403)
        .json({ message: "Token no coincide con el email" });
    }
